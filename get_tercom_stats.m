function [avg_score, best_score, best_pos, ratio , skipSolution, mapFeatures, scoreMapFeatures] = ppr_tercom(pred_pos, terrain, P_INS, meas, varargin)
...
if ~isempty(varargin)
    ...
    H_bias = (H_first_point(1:2)  - pred_pos(1,1:2));
...
centerIdx   = ceil(matrixSize/2); % initial guess index
...
score_map_NCC               = +inf(2*idx_n+1, 2*idx_e+1);
...
for i=-idx_n:1:idx_n
    for j=-idx_e:1:idx_e
        sumMAD = 0; sumMSD = 0; measHoT = zeros(1,N); mapHoT = zeros(1,N);
        for k=1:N
        ...
        score_map_NCC(idx_n + i +1, idx_e + j +1) = sumNCC/N;
...
%%
% bestLocal_MAD_deg =  [offset_MAD 0]   + [pred_pos(1,:)];
% MAD_path_deg  = bestLocal_MAD_deg(1:2) + [(dRow /3601), (dCol /3601)];
% MAD_path_index  = bestLocal_abs_diff    + [(dRow) , (dCol)];
...
cands_adaptive_NCC  = nms_botk_adaptive(score_map_NCC, rNMS);
...
for i = 1:2
    ...
    M_NCC(i) = score_map_NCC(cands_adaptive_NCC(i,1),cands_adaptive_NCC(i,2));
...
bestPos_MSD = [offset_MSD+H_bias 0]   + [pred_pos(end,:)];

offset_NCC  = ([cands_adaptive_NCC(1,:)] - centerIdx_scoreMap(:)')*resolution_deg;
bestPos_NCC = [offset_NCC+H_bias 0]   + [pred_pos(end,:)];
...
% may be used for plotting score maps
% figure()
% subplot(3,3,1)
...
% 
% subplot(3,3,3)
% imagesc(score_map_NCC);  xlim([1,2*idx_e+1]); ylim([1,2*idx_n+1]);hold on; grid on; colormap('jet'); colorbar; set(gca,'YDir','normal');
